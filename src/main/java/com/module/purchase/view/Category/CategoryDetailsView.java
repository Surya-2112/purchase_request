...
@PermitAll
public class CategoryDetailsView extends VerticalLayout implements HasUrlParameter<String> {
        ...
        @Override
        public void setParameter(BeforeEvent event, String categoryId) {

                removeAll();
                try {
                        Category category = categoryService.getCategoryById(Long.parseLong(categoryId)).orElse(null);
                        if (category == null) {
                        ...
                        if (category.isAutoRfq()) {
                                Optional<RepeatedPeriod> periodOpt = repeatedPeriodService
                                                .findByReferTypeAndReferId(RepeatedPeriodReferType.CATEGORY,
                                ...
                                if (periodOpt.isPresent()) {
                                        ...
                                        String intervalText = "Every " + period.getFrequencyPeriod() + " " +(period.getFrequencyType() != null
                                                                        ? period.getFrequencyType().name().toLowerCase() : "");
                                        ...
                                        scheduleFormLayout.addFormItem(new Span(period.getToDate() != null? period.getToDate().toString()
                                                                       : "Indefinite / No End Date"),"Sourcing End Date");

                                        scheduleFormLayout.addFormItem( new Span(period.getNextDate() != null? period.getNextDate().toString()
                                                                       : "Deactivated"),"Next Automated RFQ Run");
                        ...
                        deleteButton.addClickListener(e -> {
                                ...
                                dialog.addConfirmListener(confirmEvent -> {
                                        try {
                                                ...
                                                Notification.show(exception.getMessage(), 5000,
                                                                Notification.Position.TOP_CENTER);
                                        }
                                });
                                dialog.open();
                        ...
                        event.forwardTo(ViewName.CATEGORY.getRoute());
                        event.getUI().access(() -> {
