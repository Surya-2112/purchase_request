...
@PermitAll
public class CategoryEditView extends VerticalLayout implements HasUrlParameter<String> {
        ...
        public CategoryEditView(CategoryService categoryService, RepeatedPeriodService repeatedPeriodService, SecurityService securityService) {
        ...
        public void setParameter(BeforeEvent event, String categoryId) {
                ...
                removeAll();
                try {
                        category = categoryService.getCategoryById(Long.parseLong(categoryId)).orElse(null);
                        if (category == null) {
                        ...
                        if (category.isAutoRfq()) {
                                ...
                                if (periodOpt.isPresent()) {
                                        ...
                                        if (period.getFromDate() != null
                                                        && period.getFromDate().isBefore(LocalDate.now())) {
                        ...
                        saveButton.addClickListener(e -> {
                                try {
                                        ...
                                        if (autoRfqField.getValue()) {
                                                ...
                                                if (fromDateField.getValue().isBefore(LocalDate.now())
                                                                && !category.isAutoRfq()) {
                                                        Notification.show("Start Date cannot be a past date", 3000,
                                                                        Notification.Position.TOP_CENTER);
                                                        return;
                                                }
                                        }
